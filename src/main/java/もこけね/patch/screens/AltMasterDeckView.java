...
public class AltMasterDeckView {
    ...
    public static class UseAltTip
    {
        ...
        public static void renderAltTip(TopPanel __instance, SpriteBatch sb)
        {
            if (AbstractDungeon.player.chosenClass == CharacterEnums.MOKOUKEINE && AbstractDungeon.player instanceof MokouKeine)
            {
                ...
                TipHelper.renderGenericTip(1550.0F * Settings.scale, (float)Settings.HEIGHT - 120.0F * Settings.scale, TopPanel.LABEL[1] + " (" + InputActionSet.masterDeck.getKeyString() + ")", uiStrings.TEXT[1] + ((MokouKeine) AbstractDungeon.player).getOtherPlayerName() + uiStrings.TEXT[2]);
            }
        }
    ...
    public static class OpenAlt
    {
        ...
        {
            if (!clicked[0] && AbstractDungeon.player.chosenClass == CharacterEnums.MOKOUKEINE && InputHelper.justClickedRight && __instance.deckHb.hovered)
            {
                clicked[0] = true;
                openAlt = true;
            }
    ...
    public static class ControllerUseAlt
    {
        ...
        {
            if (openAlt && AbstractDungeon.player instanceof MokouKeine)
            {
                deck[0] = ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck;
            }
        }
    ...
    public static class UpdateAltPositions
    {
        ...
        public static void updateAlt(MasterDeckViewScreen __instance, @ByRef(type="java.util.ArrayList") Object[] cards)
        {
            if (openAlt && AbstractDungeon.player instanceof MokouKeine)
            {
                cards[0] = ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.group;
            }
        }
    ...
    public static class UpdateAltClick
    {
        ...
        public static SpireReturn openAlt(MasterDeckViewScreen __instance, AbstractCard hovered, @ByRef(type="com.megacrit.cardcrawl.cards.AbstractCard") Object[] clickStartedCard)
        {
            if (openAlt && AbstractDungeon.player instanceof MokouKeine)
            {
                CardCrawlGame.cardPopup.open(hovered, ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck);
    ...
    public static class CalcAltBounds
    {
        ...
        public static SpireReturn calcAlt(MasterDeckViewScreen __instance)
        {
            if (openAlt && AbstractDungeon.player instanceof MokouKeine)
            {
                if (((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.size() > 10) {
                    int scrollTmp = ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.size() / 5 - 2;
                    if (((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.size() % 5 != 0) {
                        ++scrollTmp;
                    }
                ...
                ReflectionHacks.setPrivate(__instance, MasterDeckViewScreen.class, "prevDeckSize", ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.size());
                return SpireReturn.Return(null);
            }
            return SpireReturn.Continue();
        }
    ...
    public static class RenderAlt
    {
        ...
        public static SpireReturn rendering(MasterDeckViewScreen __instance, SpriteBatch sb)
        {
            if (openAlt && AbstractDungeon.player instanceof MokouKeine) {
                ...
                if (hoveredCard == null) {
                    ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.renderMasterDeck(sb);
                } else {
                    ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.renderMasterDeckExceptOneCard(sb, hoveredCard);
                    hoveredCard.renderHoverShadow(sb);
                    hoveredCard.render(sb);
                }

                ((MokouKeine) AbstractDungeon.player).otherPlayerMasterDeck.renderTip(sb);// 345
                FontHelper.renderDeckViewTip(sb, ((MokouKeine) AbstractDungeon.player).getOtherPlayerName() + uiStrings.TEXT[0], 96.0F * Settings.scale, Settings.CREAM_COLOR);
